...
class SessionManager:
    ...
    def get_user_active_fast(request):
        ...
        if active_fast_id:
            try:
                fast = Fast.objects.get(id=active_fast_id, user=request.user, end_time__isnull=True)
                return fast
    ...
    def acquire_fast_lock(user, fast_id=None):
        ...
        try:
            if fast_id:
                ...
                return Fast.objects.with_lock().filter(user=user, end_time__isnull=True).first()
        except Fast.DoesNotExist:
            return None