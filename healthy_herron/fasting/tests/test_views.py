...
class StartFastViewTest(TestCase):
    ...
    def test_start_fast_post_success(self):
        ...
        post_data = {"start_time": timezone.now().strftime("%Y-%m-%dT%H:%M")}
        ...
        assert Fast.objects.filter(user=self.user).count() == 1

        # Success message should be displayed
        messages = list(get_messages(response.wsgi_request))
        assert any("Fast started successfully" in str(m) for m in messages)
...
class EndFastViewTest(TestCase):
    ...
    def test_end_fast_post_success(self):
        ...
        # Success message should be displayed
        messages = list(get_messages(response.wsgi_request))
        assert any("Fast completed successfully" in str(m) for m in messages)
...
class FastListViewTest(TestCase):
    ...
    def test_fast_list_pagination(self):
        ...
        assert len(response.context["fasts"]) == 20
    ...
    def test_fast_list_statistics(self):
        ...
        assert response.status_code == 200
        assert response.context["total_fasts"] == 2
        assert response.context["completed_fasts"] == 1
        assert response.context["active_fasts"] == 1
...
class FastUpdateViewTest(TestCase):
    ...
    def test_fast_update_post_success(self):
        ...
        fast.refresh_from_db()
        assert fast.emotional_status == "energized"
        assert fast.comments == "Updated comment"
...
class FastDeleteViewTest(TestCase):
    ...
    def test_fast_delete_post_success(self):
        ...
        assert not Fast.objects.filter(pk=fast.pk).exists()
