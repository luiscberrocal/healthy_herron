...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_count == 0:
            self.stdout.write(
                self.style.WARNING("No fasts found for archival.")
            )
            return
        
        self.stdout.write(
            self.style.WARNING(
                f"Found {total_count} fasts eligible for archival"
            )
        )
        ...
        confirm = input(f"Are you sure you want to archive {total_count} fast records? (yes/no): ")
        if confirm.lower() != 'yes':
            self.stdout.write(
                self.style.ERROR("Archival cancelled by user.")
            )
            return
