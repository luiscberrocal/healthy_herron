...
Templates Status:
  ✅ plan-template.md - verified Constitution Check gate present
...
Health tracking application built with Django, HTMX, and Tailwind CSS. This constitution defines the non-negotiable principles and standards for all development work.
...
All new features MUST follow test-first development approach. Write tests before implementation. Tests must validate business requirements and edge cases. Maintain high test coverage focusing on critical paths. Use pytest with class-based test organization. Tests must be fast, isolated, and deterministic.

**Rationale**: Test-first development ensures code meets requirements, prevents regressions, and serves as living documentation. Writing tests first forces clear thinking about interfaces and behavior before implementation details distract from design quality.

### VII. Progressive Enhancement

All features must work without JavaScript as a baseline. Enhance with HTMX for improved user experience. Server-side rendering is the default. Client-side enhancements must degrade gracefully. No Single Page Application (SPA) patterns.

**Rationale**: Progressive enhancement ensures accessibility, improves SEO, reduces complexity, and provides resilience against client-side failures. Server-side rendering with HTMX enhancement delivers modern interactivity without SPA complexity.
